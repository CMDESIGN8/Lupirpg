  ...
  <div className="inventory-container">
    <div className="inventory-box">
      ...
      <div className="inventory-grid">
        {inventory.length > 0 ? (
          inventory.map(item => {
            // Detectamos si el item viene anidado en `items` o plano
            ...
            return (
              <div key={item.id} className={`inventory-item ${item.is_equipped ? 'item-equipped' : ''}`}>
                <div className="item-image-wrapper">
                  <img
                    src={itemAssets[actualItem.name] || "/assets/items/default.png"}
                    alt={actualItem.name}
                    className="item-image"
                  />
                </div>
                <div className="item-header">
                  <h3 className="item-name">{actualItem.name}</h3>
                  <p className="item-stats">
                    Bonificación: {actualItem.skill_bonus} 
                    <span className="bonus-value"> +{actualItem.bonus_value}</span>
                  </p>
                </div>
                ...
                <div className="item-actions">
                  {!item.is_equipped ? (
                    <button 
                      onClick={() => handleEquipItem(item.id, actualItem.skill_bonus)} 
                      disabled={loading} 
                      className="equip-btn"
                    >
                      <Sword size={16} />
